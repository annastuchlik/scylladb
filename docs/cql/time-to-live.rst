...
ScyllaDB provides two mechanisms for automatically deleting expired data.

**Per-write TTL** (also supported by Apache Cassandra) attaches an expiration
duration to individual column writes. TTL is measured in seconds and is set at
write time via ``USING TTL``. Because each write carries its own TTL, different
columns in the same row can expire at different times.
...
the entire row. The whole row expires together, regardless of when its
individual columns were written, and the expiration time can be changed by
updating that one column.
...
Or per-row TTL can be disabled so that rows never expire:
...
* Atomic, whole-row expiration is important and partial expiry is undesirable.

It is possible to use both mechanisms in the same table or even the same row,
but this is rarely necessary and can be confusing.
